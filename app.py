...
@st.cache_resource
def train_models(df):
    """Preprocesses data, trains models with hyperparameter tuning, and returns them."""
    ...
    # --- XGBoost with Hyperparameter Tuning and SMOTE ---
    ...
    return best_xgb_pipeline, X_test, y_test, X_train
...
try:
    ...
    if df is not None:
        with st.spinner("Training advanced models... This may take a minute."):
            pipeline, X_test, y_test, X_train = train_models(df)
        ...
        with tab2:
            ...
            with st.form("prediction_form"):
                ...
                # ... (omitted for brevity, but they are the same as before) ...
        ...
        with tab3:
            ...
            if selected_idx is not None:
                ...
                st.write("The plot below shows how each feature contributed to the final prediction for the selected customer. Red features increase churn risk, blue features decrease it.")
                fig_force, ax_force = plt.subplots()
                shap.force_plot(explainer.expected_value, shap_values_single, X_test.iloc[[idx_pos]], matplotlib=True, show=False)
